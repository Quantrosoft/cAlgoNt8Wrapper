...
using System.Linq;
using TdsCommons;
using TdsCommons.Mc64Functions;
...
namespace RobotLib
{
    ...
    public delegate bool DelegateCloseTrade(Position pos);
    ...
    public class Drawings
    {
        ...
        private int mPrevNoOverlayBarCount;
        private double mPrevOverlayData;
        private double mPrevNoOverlayData;
                ...
                if (mBot.Chart.LastVisibleBarIndex >= 0)
                    if (mBot.Chart.FirstVisibleBarIndex < mBot.Chart.LastVisibleBarIndex)
                        if (mBot.Chart.FirstVisibleBarIndex < mBot.Chart.Bars.Count)
                        {
                            ...
                            var firstVisibleBarTime = mBot.Chart.Bars.OpenTimes[mBot.Chart.FirstVisibleBarIndex];
                            for (int i = mBot.Chart.LastVisibleBarIndex;
                               i >= mBot.Chart.FirstVisibleBarIndex;
                               i--)
                            {
                                var dataNdx = mBot.Chart.LastVisibleBarIndex - i;
                                if (data.Last(dataNdx).IsValid
                                   && dataNdx < data.Length
                                   && data.Last(dataNdx).Time >= firstVisibleBarTime)
                                {
                                    CoFu.Max(ref maxVal, data.Last(dataNdx).Bid);
                                    CoFu.Min(ref minVal, data.Last(dataNdx).Bid);
                                }
                            }

                            if (0 != maxVal && 0 != minVal && minVal != double.MaxValue)
                            {
                                ...
                                var noOverlays = mBot.Chart.FindAllObjects(ChartObjectType.TrendLine);
                                foreach (var noOverlay in noOverlays)
                                {
                                    if (noOverlay.Name.Contains("NoOverlay" + name))
                                        mBot.Chart.RemoveObject(noOverlay.Name);
                                }
                                ...
                                for (int i = 0;
                                   0 != drawFact
                                   && i < data.Length
                                   && data.Last(i).Time > mBot.Chart.Bars.OpenTimes[mBot.Chart.FirstVisibleBarIndex];
                                   i++)
                                {
                                    var prev = data.Last(i + 1);
                                    var current = data.Last(i);
                                    if (!double.IsNaN(current) && !double.IsNaN(prev) && data.Last(i).IsValid)
                                        mBot.Chart.DrawTrendLine("NoOverlay" + name + (i - mBot.Chart.FirstVisibleBarIndex),
                                           data.Last(i + 1).Time,
                                           mBot.Chart.BottomY + (prev - minVal) / drawFact,
                                           0 == i ? currentTime : data.Last(i).Time,
                                           mBot.Chart.BottomY + (current - minVal) / drawFact,
                                           color).IsInteractive = true;
                                }
                            }
                        }
        ...
        public void DrawNoOverlay(
           RingbufferTimePrice dataRb,
           string name,
           Color color,
           int thickness,
           Xaxis xaxis = Xaxis.None)
        {
            var isNewChartBar = mPrevNoOverlayBarCount != mBot.Bars.Count;
            var isInit = 0 == mPrevNoOverlayBarCount;
                ...
                if (mBot.Chart.LastVisibleBarIndex >= 0)
                    if (mBot.Chart.FirstVisibleBarIndex < mBot.Chart.LastVisibleBarIndex)
                        if (mBot.Chart.FirstVisibleBarIndex < mBot.Chart.Bars.Count)
                        {
                            ...
                            {
                                var dataNdx = mBot.Chart.LastVisibleBarIndex - i;
                                if (dataNdx < dataRb.Count
                                   && dataRb.Last(dataNdx).Item1 >= firstVisibleBarTime)
                                {
                                    CoFu.Max(ref maxVal, dataRb.Last(dataNdx).Item2);
                                    CoFu.Min(ref minVal, dataRb.Last(dataNdx).Item2);
                                }
                            }

                            var altLowest = dataRb.Lowest();

                            if (0 != maxVal && 0 != minVal && minVal != double.MaxValue)
                            {
                                var yVal = 0.0;
                                mBot.Chart.RemoveObject("Xaxis");
                                switch (xaxis)
                                {
                                    case Xaxis.Bottom:
                                    minVal = 0;
                                    yVal = mBot.Chart.BottomY;
                                    break;

                                    case Xaxis.Middle:
                                    yVal = (mBot.Chart.TopY + mBot.Chart.BottomY) / 2;
                                    break;

                                    case Xaxis.Top:
                                    maxVal = 0;
                                    yVal = mBot.Chart.TopY;
                                    break;
                                }
                                mBot.Chart.DrawHorizontalLine("Xaxis", yVal, Color.White, 1, LineStyle.DotsRare);
                                ...
                                for (int i = 0;
                                   0 != drawFact
                                   && i < dataRb.Count - 1
                                   && dataRb.Last(i).Item1 > mBot.Chart.Bars.OpenTimes[mBot.Chart.FirstVisibleBarIndex];
                                   i++)
                                {
                                    var prev = dataRb.Last(i + 1).Item2;
                                    var current = dataRb.Last(i).Item2;
                                    if (!double.IsNaN(current)
                                       && !double.IsNaN(prev)
                                       && 0 != mPrevNoOverlayData)

                                        mBot.Chart.DrawTrendLine("NoOverlay" + name
                                              + (mBot.Chart.Bars.Count % mBot.Chart.MaxVisibleBars + (isNewChartBar ? -1 : 0)).ToString(),
                                           isInit ? dataRb.Last(i + 1).Item1 : mBot.Bars.OpenTimes.Last(isNewChartBar ? 1 : 0),
                                           mBot.Chart.BottomY + ((isInit
                                              ? dataRb.Last(i + 1).Item2
                                              : mPrevNoOverlayData) - minVal) / drawFact,
                                           dataRb.Last(i).Item1,
                                           mBot.Chart.BottomY + (dataRb.Last(i).Item2 - minVal) / drawFact,
                                           color, thickness).IsInteractive = true;
                                ...
                                if (isInit || isNewChartBar)
                                    mPrevNoOverlayData = dataRb.Last(0).Item2;
                            }
                        }
            mPrevNoOverlayBarCount = mBot.Bars.Count;
        }
    ...
    public class IndiMembers
    {
        public bool IsAfterLastBar;
        public int TfNdx;
        public int PrevTfNdx;
        public Bars TfBars;
        public MeBars MeBars;
        public NumericSeries CalcSeries;
        public double Price;
    }
    ...
    public class HedgePosition
    {
        ...
        public const string ReverseId = "Reverse;";
        public const string MainFreezeId = "MainFreeze;";
        public const string WeekendFreezeId = "WeekendFreeze;";

        public Position MainPosition;
        public Position FreezePosition;

        public double FreezePriceOffset
        {
            get
            {
                return mFreezePriceOffset +
                   (null == FreezePosition
                   ? 0
                   : FreezePosition.CurrentPrice - FreezePosition.EntryPrice);
            }
        }

        public double FreezeCorrectedLastEntryPrice
        {
            get
            {
                return LastEntryPrice + FreezePriceOffset;
            }
        }
        ...
        public double Profit
        {
            get
            {
                return (null == MainPosition ? 0 : MainPosition.NetProfit)
                   + (null == FreezePosition ? 0 : FreezePosition.NetProfit)
                   + mFreezeProfitOffset;
            }
        }
        ...
        public double MaxVolume
        {
            get
            {
                if (null != MainPosition && null != FreezePosition)
                    return Math.Max(MainPosition.VolumeInUnits, FreezePosition.VolumeInUnits);
                else if (null != MainPosition)
                    return MainPosition.VolumeInUnits;
                else if (null != FreezePosition)
                    return FreezePosition.VolumeInUnits;
                else
                    return 0;

            }
        }
        public double GridClose;   // to be replaced by Doubles[0] Doubles[GridClose] where GridClose is an const int = 0
        public double GridMiddle;  // to be replaced by Doubles[1]
        public double GridOpen;    // to be replaced by Doubles[2]
        public bool IsProfitEarned;
        public int SharesCount;
        public int FreezeOpenBarCount;
        public int LastModifiedBar;
        ...
        private double mMainMarginAfterOpen;
        private double mFreezeMarginAfterOpen;
        private DelegateOpenTrade mOpenTrade;
        private DelegateCloseTrade mCloseTrade;
        private double mFreezeProfitOffset;
        private double mFreezePriceOffset;
        #endregion

        public HedgePosition() { } // dummy cTor

        public HedgePosition(Robot bot, Symbol symbol, string label, DelegateOpenTrade openTrade, DelegateCloseTrade closeTrade)
        {
            mBot = bot;
            mSymbol = symbol;
            mLabel = label;
            mOpenTrade = openTrade;
            mCloseTrade = closeTrade;
        }

        public bool DoMainOpen(bool isLong, double volume, double inheritedFreezePriceOffset = 0, string labelExt = MainId)
        {
            mIsLong = isLong;

            if (null == MainPosition)
            {
                MainPosition = mOpenTrade(
                   mSymbol,
                   mIsLong ? TradeType.Buy : TradeType.Sell,
                   mIsLong ? TradeDirections.Long : TradeDirections.Short,
                   "",
                   mLabel + labelExt,
                   mSymbol.NormalizeVolumeInUnits(volume));

                if (null != MainPosition)
                {
                    mMainMarginAfterOpen = mBot.Account.Margin;
                    mMainMarginAfterOpen = 0;
                    mFreezePriceOffset = inheritedFreezePriceOffset;
                    LastModifiedBar = mBot.Bars.Count;
                    LastEntryPrice = MainPosition.EntryPrice - FreezePriceOffset;
                    LastModifiedTime = mBot.Time;
                }
            }
            else
                Debugger.Break();

            return null != MainPosition;
        }

        public bool DoMainClose(ref TimeSpan MinOpenDuration,
           ref TimeSpan AvgOpenDurationSum,
           ref int OpenDurationCount,
           ref TimeSpan MaxOpenDuration,
           bool isUtc = true)
        {
            bool result = false;
            if (null != MainPosition)
            {
                //mFreezeProfitOffset += MainPosition.NetProfit;
                //mFreezePriceOffset += MainPosition.CurrentPrice - MainPosition.EntryPrice;
                result = mCloseTrade(MainPosition);
                MainPosition = null;
            }
            else
                Debugger.Break();
        ...
        public bool DoFreezeOpen(double volume = 0)
        {
            if (null == FreezePosition)
                return DoFreeze(MainFreezeId, volume);
            else
                Debugger.Break();
            return false;
        }

        public bool DoFreezeClose(ref TimeSpan MinOpenDuration,
           ref TimeSpan AvgOpenDurationSum,
           ref int OpenDurationCount,
           ref TimeSpan MaxOpenDuration,
           bool isUtc = true)
        {
            ...
            if (null != FreezePosition)
            {
                mFreezeProfitOffset += FreezePosition.NetProfit;
                mFreezePriceOffset += FreezePosition.CurrentPrice - FreezePosition.EntryPrice;
                result = mCloseTrade(FreezePosition);
                FreezePosition = null;
            }
            else
                Debugger.Break();
        ...
        public void InitFrozenHistoryPosition(HistoricalTrade histPos)
        {
            mFreezeProfitOffset += histPos.NetProfit;
            mFreezePriceOffset += histPos.ClosingPrice - histPos.EntryPrice;
        }

        public bool DoModifyVolume(double volume, double currentOpenPrice)
        {
            LastModifiedBar = mBot.Bars.Count;
            LastEntryPrice = currentOpenPrice - FreezePriceOffset;
            LastModifiedTime = mBot.Time;
            return MainPosition.ModifyVolume(volume).IsSuccessful;
        }

        public bool DoExchangeAndFreezeClose(ref TimeSpan MinOpenDuration,
           ref TimeSpan AvgOpenDurationSum,
           ref int OpenDurationCount,
           ref TimeSpan MaxOpenDuration,
           bool isUtc = true)
        ...
        public bool DoBothClose(ref TimeSpan MinOpenDuration,
           ref TimeSpan AvgOpenDurationSum,
           ref int OpenDurationCount,
           ref TimeSpan MaxOpenDuration,
           bool isUtc = true)
        {
            ...
            if (null != MainPosition)
            {
                DoMainClose(ref MinOpenDuration,
                   ref AvgOpenDurationSum,
                   ref OpenDurationCount,
                   ref MaxOpenDuration,
                   isUtc);
                MainPosition = null;
            }
            ...
            if (null != FreezePosition)
            {
                DoFreezeClose(ref MinOpenDuration,
                   ref AvgOpenDurationSum,
                   ref OpenDurationCount,
                   ref MaxOpenDuration,
                   isUtc);
        ...
        public bool CloseFrozenAndModifyMain(double volume,
           ref TimeSpan MinOpenDuration,
           ref TimeSpan AvgOpenDurationSum,
           ref int OpenDurationCount,
           ref TimeSpan MaxOpenDuration,
           bool isUtc = true)
        {
            ...
            MainPosition.ModifyVolume(volume);
            retVal = DoFreezeClose(ref MinOpenDuration,
               ref AvgOpenDurationSum,
               ref OpenDurationCount,
               ref MaxOpenDuration,
               isUtc);
        ...
        public bool CloseMainAndModifyFrozen(double volume,
           ref TimeSpan MinOpenDuration,
           ref TimeSpan AvgOpenDurationSum,
           ref int OpenDurationCount,
           ref TimeSpan MaxOpenDuration,
           bool isUtc = true)
        {
            ...
            FreezePosition.ModifyVolume(volume);
            retVal = DoMainClose(ref MinOpenDuration,
               ref AvgOpenDurationSum,
               ref OpenDurationCount,
               ref MaxOpenDuration,
               isUtc);
        ...
        public bool Reverse(double volume,
           ref TimeSpan MinOpenDuration,
           ref TimeSpan AvgOpenDurationSum,
           ref int OpenDurationCount,
           ref TimeSpan MaxOpenDuration,
           bool isUtc = true)
        {
            bool retVal = false;
            if (null == MainPosition && null == FreezePosition
               || null != MainPosition && null != FreezePosition)
            {
                Debugger.Break();
                return retVal;
            }
            ...
            retVal = DoMainOpen(mIsLong, volume, mFreezePriceOffset, ReverseId);
            return retVal;
        }
        ...
        public bool Exchange()
        {
            ...
            var backup = MainPosition;
            MainPosition = FreezePosition;
            FreezePosition = backup;

            return true;
        }

        public bool DoWeekEndFreeze(double volume = 0)
        {
            // if alreay main frozen, don't freeze again
            if (null == FreezePosition)
                return DoFreeze(WeekendFreezeId, volume);
            else
                Debugger.Break();
            return false;
        }

        private bool DoFreeze(string labelExtension, double volume)
        {
            ...
            if (null == FreezePosition)
            {
                result = mOpenTrade(
                   ...
                   !mIsLong ? TradeDirections.Long : TradeDirections.Short,
                   "",
                   mLabel + labelExtension,
                   mSymbol.NormalizeVolumeInUnits(0 == volume ? MainPosition.VolumeInUnits : volume));

                if (null != result)
                {
                    FreezePosition = result;
                    mFreezeMarginAfterOpen = mBot.Account.Margin;
                }
            }
            else
                Debugger.Break();

            return null != FreezePosition;
        }
    }
    ...
    public class Speed
    {
        ...
        private Bars mBars;
        public int Slots;
        public Quote Highest;
        public Quote Lowest;
        public List<Quote> LowExtremes;
        public List<Quote> HighExtremes;
        public List<Quote> LowFallouts;
        public List<Quote> HighFallouts;

        public DateTime StartTime
        {
            get { return mHighs.First().Item1; }
        }

        public bool IsValid
        {
            get { return mHighs.Count == mHighs.Size; }
        }

        private RingbufferTimePrice mHighs;
        private RingbufferTimePrice mLows;
        private int mPrevBar;
        private int mTfSeconds;
        #endregion

        public Speed(Bars bars, int slots)
        {
            mBars = bars;
            Slots = slots;
            mTfSeconds = slots * mBars.TimeFrame.GetHashCode() * SEC_PER_MINUTE;
            Highest.Bid = double.MinValue;
            Lowest.Bid = double.MaxValue;
            mHighs = new RingbufferTimePrice(Slots);
            mLows = new RingbufferTimePrice(Slots);
        }

        public void OnTick(int index, DateTime time)
        {
            Lowest.IsNewExtreme = Highest.IsNewExtreme = Lowest.IsNewFallout = Highest.IsNewFallout = false;
            var absNdx = mBars.Count - index - 1;
            var high = mBars.HighPrices.Last(index);
            var low = mBars.LowPrices.Last(index);
            if (0 == index)
            {
                CoFu.Max(ref high, mBars.HighPrices.Last(1));
                CoFu.Min(ref low, mBars.LowPrices.Last(1));
            }
            var openTime = mBars.OpenTimes.Last(index);

            if (absNdx != mPrevBar) // new bar
            {
                if (0 == index)
                {
                    if (CoFu.Max(ref high, mBars.HighPrices.Last(1)))
                        openTime = time;
                    if (CoFu.Min(ref low, mBars.LowPrices.Last(1)))
                        openTime = time;
                }

                mHighs.Add((time, high), out int _, out (DateTime, double) fallOutHigh);
                mLows.Add((time, low), out int _, out (DateTime, double) fallOutLow);
                //mHighLowDt.Add(openTime, out int _, out _);
                //mStartDt.Add(openTime);

                if (fallOutHigh.Item2 == Highest.Bid)
                {
                    var highestTuple = mHighs.Highest();
                    Highest.Time = highestTuple.Item1;
                    Highest.Bid = highestTuple.Item2;
                    Highest.IsNewFallout = true;

                    if (null != LowFallouts || null == HighFallouts)
                    {
                        LowFallouts = null;
                        HighFallouts = new List<Quote>
                  {
                     Highest,
                  };
                    }
                    else
                        HighFallouts.Add(Highest);
                }

                if (fallOutLow.Item2 == Lowest.Bid)
                {
                    var lowestTuple = mLows.Lowest();
                    Lowest.Time = lowestTuple.Item1;
                    Lowest.Bid = lowestTuple.Item2;
                    Lowest.IsNewFallout = true;

                    if (null != HighFallouts || null == LowFallouts)
                    {
                        HighFallouts = null;
                        LowFallouts = new List<Quote>
                  {
                     Lowest,
                  };
                    }
                    else
                        LowFallouts.Add(Lowest);
                }
            }

            mHighs.Swap((time, high));
            mLows.Swap((time, low));

            if (CoFu.Max(ref Highest.Bid, high))
            {
                Highest.Time = time;
                Highest.BarIndex = absNdx;
                Highest.IsNewExtreme = true;

                if (null != LowExtremes || null == HighExtremes)
                {
                    LowExtremes = null;
                    HighExtremes = new List<Quote>
               {
                  Highest,
               };
                }
                else
                    HighExtremes.Add(Highest);
            }

            if (CoFu.Min(ref Lowest.Bid, low))
            {
                Lowest.Time = time;
                Lowest.BarIndex = absNdx;
                Lowest.IsNewExtreme = true;

                if (null != HighExtremes || null == LowExtremes)
                {
                    HighExtremes = null;
                    LowExtremes = new List<Quote>
               {
                  Lowest,
               };
                }
                else
                    LowExtremes.Add(Lowest);
            }

            mPrevBar = absNdx;
        }

        public double UpSpeed(DateTime time, double price)
        {
            var timeDiff = time.ToNativeSec() - Lowest.Time.ToNativeSec();
            if (timeDiff < 3600)
                return 0;

            if (price / Lowest.Bid - 1 < 0 || timeDiff < 0)
                Debugger.Break();

            return 1e6 * (price / Lowest.Bid - 1)
               / Math.Sqrt(timeDiff);
        }

        public double DownSpeed(DateTime time, double price)
        {
            var timeDiff = time.ToNativeSec() - Highest.Time.ToNativeSec();
            if (timeDiff < 3600)
                return 0;

            if (price / Highest.Bid - 1 > 0 || timeDiff < 0)
                Debugger.Break();

            return 1e6 * (price / Highest.Bid - 1)
               / Math.Sqrt(timeDiff);
        }

        public double TfSpeed()
        {
            var priceDiff = Highest.Bid - Lowest.Bid;
            if (priceDiff < 0)
                Debugger.Break();

            return 1e6 * (priceDiff / Lowest.Bid) / Math.Sqrt(mTfSeconds);
        }

        public bool IsUpperHalf(double price)
        {
            var priceHalf = (Highest.Bid + Lowest.Bid) / 2;
            return price > priceHalf;
        }
    }
    ...
    public abstract class AbstractRobot : IRobot
    {
        ...
        public GetCurrentTime GetCurrentTimeDelegate()
        {
        ...
        public void PrintComment(string version, string comment, int avgSpreadPts, bool normNyTime = false)
        {
            ...
            mRobot.Chart.DrawStaticText("Comment3",
               "\n\n" + cCommentTab + "MaxLot: " + ConvertUtils.DoubleToString(mRobot.Symbol.VolumeInUnitsToQuantity(mRobot.Symbol.VolumeInUnitsMax), 2)
               + ", Spread: " + ConvertUtils.IntegerToString(SpreadInPoints(mRobot.Symbol)) // IntegerToString() needed for MT5 to avoid warnings
        ...
        protected virtual void TickDataManagement()
        {
...
#if !TDS && CTRADER
        static public void InitMeIndicator(
           Indicator ctIndi,
           Symbol Symbol,
           string symbolName,
           int TfMinutes,
        ...
        {
            mIndi.PrevTfNdx = -1;
            mIndi.IsAfterLastBar = false;

            var symbol = Symbol;
            if ("-1" != symbolName)
                symbol = ctIndi.Symbols.GetSymbol(symbolName);

            if (-1 == TfMinutes)
                TfMinutes = ctIndi.Bars.TimeFrame.GetHashCode();

            var tfSecs = TfMinutes * SEC_PER_MINUTE;
            var ctTf = Secs2Tf(tfSecs, out bool isExact);
            if (!isExact)
            {
                ctIndi.Print("Indicator parameter TfMinutes ("
                   + TfMinutes
                   + ") must match an existing cTrader timeframe");
                return;
            }

            mIndi.TfBars = ctIndi.MarketData.GetBars(ctTf, symbol.Name);
            //mIndi.MeBars = new MeBars(symbol.Name, tfSecs, symbol.TickSize);
        }

        static public bool CalcMeIndicator(Indicator ctIndi,
           IndiMembers mIndi,
           IndicatorSeries Prices,
           int ctNdx,
           IndicatorDataSeries Result,
           bool isInit)
        {
            if (null == mIndi.TfBars)
                return false;

            mIndi.CalcSeries = null;

            if (isInit)
                mIndi.TfNdx = ctNdx;
            else if (ctIndi.IsLastBar || (mIndi.TfBars.Count > 0
                  && mIndi.TfBars.OpenTimes[mIndi.TfBars.OpenTimes.Count - 1] < ctIndi.Bars.OpenTimes[ctNdx]))
            {
                mIndi.IsAfterLastBar = true;
                mIndi.TfNdx = mIndi.TfBars.Count - 1;
            }
            else
                mIndi.TfNdx = mIndi.TfBars.ToList().FindIndex(x
                   => x.OpenTime >= ctIndi.Bars.OpenTimes[ctNdx]);

            switch (Prices)
            {
                case IndicatorSeries.Open:
                mIndi.Price = mIndi.TfBars.OpenPrices[mIndi.TfNdx];
                mIndi.CalcSeries = mIndi.MeBars.OpenPrices;
                break;
                case IndicatorSeries.High:
                mIndi.Price = mIndi.TfBars.HighPrices[mIndi.TfNdx];
                mIndi.CalcSeries = mIndi.MeBars.HighPrices;
                break;
                case IndicatorSeries.Low:
                mIndi.Price = mIndi.TfBars.LowPrices[mIndi.TfNdx];
                mIndi.CalcSeries = mIndi.MeBars.LowPrices;
                break;
                case IndicatorSeries.Close:
                mIndi.Price = mIndi.TfBars.ClosePrices[mIndi.TfNdx];
                mIndi.CalcSeries = mIndi.MeBars.ClosePrices;
                break;
                case IndicatorSeries.HiLo2:
                mIndi.Price = (mIndi.TfBars.HighPrices[mIndi.TfNdx] + mIndi.TfBars.LowPrices[mIndi.TfNdx]) / 2;
                mIndi.CalcSeries = mIndi.MeBars.HiLo2Prices;
                break;
            }

            mIndi.MeBars.AddBar(mIndi.TfNdx, mIndi.CalcSeries, mIndi.Price,
               ctIndi.IsLastBar ? ctIndi.Time : mIndi.TfBars.OpenTimes[mIndi.TfNdx]);  // copy bar from cTrader...

            var retVal = true;
            if (!isInit
                  && !mIndi.IsAfterLastBar
                  && mIndi.PrevTfNdx == mIndi.TfNdx
                  && ctNdx > 0)
            {
                Result[ctNdx] = Result[ctNdx - 1];
                retVal = false;
            }
            mIndi.PrevTfNdx = mIndi.TfNdx;
           ...
           bool isInit)
        {
            ...
            mIndi.MeBars.AddBar(mIndi.TfNdx,
               mIndi.TfBars.OpenPrices[mIndi.TfNdx],
               mIndi.TfBars.HighPrices[mIndi.TfNdx],
               mIndi.TfBars.LowPrices[mIndi.TfNdx],
               mIndi.TfBars.ClosePrices[mIndi.TfNdx],
               ctIndi.IsLastBar ? ctIndi.Time : mIndi.TfBars.OpenTimes[mIndi.TfNdx]);  // copy bar from cTrader...
